...
| D-07 | 장부업체 직원 결제는 월 한도 장부를 유지하고, 장부업체 외 일반 사용자는 식당 등록 상품을 키움페이로 직접 결제 | 2026-07-21 PG 전환 결정 |
| D-08 | 키움페이 서버 해시·통합결제창·결과통지·2단계 취소 API만 사용하며 브라우저 리다이렉트만으로 주문을 확정하지 않음 | 다우데이타/키움페이 연동 규격 |
| D-09 | 식권 구매·통합 QR 결제의 계정 식별은 요청 본문의 `account_id`가 아니라 검증된 Bearer 토큰의 사용자 identity를 사용 | 클라이언트 지정 계정의 IDOR 방지 보안 결정 |
| D-10 | 식권 상품의 100% 할인은 허용하지 않음. 키움페이 승인 금액은 양의 정수 KRW여야 하므로 할인율은 100% 미만이어야 함 | 키움페이 결제 제약 및 결제/발급 정합성 |
