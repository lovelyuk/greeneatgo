...
- [x] 기간형 이벤트 식권 상품 등록·노출상태·앱 D-day·결제 직전 재검증 구현
- [x] 상품 이미지 1:1 크롭·800×800 WebP 서버 정규화·교체 파일 정리 구현
- [x] 업체·식당 관리자 본인 이름 수정 API/UI 구현
- [x] FCM 기기 토큰 등록·대상별 수동 공지·500개 배치 발송·발송 이력 UI 구현
- [x] Flutter 알림 권한·토큰 갱신·전면/백그라운드 수신·로그아웃 토큰 해제 구현
- [ ] Supabase에 `0020_voucher_product_events.sql` 배포
- [ ] Supabase에 `0021_app_users_self_update_hardening.sql` 배포
- [ ] Supabase에 `0022_push_notifications.sql` 배포
- [ ] Firebase Android 앱 및 API 서버 서비스 계정 환경변수 설정 후 실기기 FCM E2E
