...
### Kiwoom 결제 콜백 출시 차단 항목

현재 결제 알림은 CPID·주문번호·금액·결제수단·거래번호 검증과 IP 허용목록을
사용하며, `X-Forwarded-For`는 오른쪽 끝의 유효한 IP만 해석합니다. 그러나 프록시가
해당 헤더를 신뢰 가능하게 덮어쓴다는 경계는 배포 환경에 의존합니다. **Kiwoom이
공식 서명 검증 또는 서버 간 거래조회 인증 사양을 제공하고 구현·검증하기 전에는
최종 콜백 인증이 완료된 것이 아니므로 운영 배포를 진행하면 안 됩니다.** 문서에
없는 토큰·서명 API를 추측해 구현하지 않습니다.
