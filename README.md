...
- `customer`: 키움페이로 개인 식권을 구매하고 매장 QR에서 1장씩 사용
...
- 일반 사용자는 식당이 등록한 식권 패키지를 키움페이로 구매합니다.
...
- `SUPABASE_SERVICE_ROLE_KEY`, 키움페이 비밀키, Firebase Admin 서비스 계정, SendGrid 키는 서버에만 둡니다.
...
키움페이의 결과 알림은 최종 승인에 대한 권위 있는 통지입니다. 서버는 인그레스가
소켓 피어로 전달하는 실제 발신 IP(`request.client.host`)를 허용목록과 대조하며
(`X-Forwarded-For`는 내부 프록시 홉이 덧붙어 신뢰하지 않고, 로컬·테스트 피어일 때만
참고), CPID·주문번호·금액·
결제수단·DAOUTRX를 검증한 뒤, 하나의 원자적 데이터베이스 RPC에서 알림 inbox 저장,
저장 주문 사실 재검증, 결제 완료 및 식권/보조금 자산 발급을 즉시 처리합니다. 동일한
알림은 중복 발급 없이 성공하고 충돌하는 알림은 실패하며, 트랜잭션이 커밋된 뒤에만
`SUCCESS`를 응답합니다. `RESULTCODE`가 있으면 키움페이 성공 코드 `0000`만 승인으로
처리하고, 이전 정상 알림과의 호환성을 위해 필드가 없는 형식도 허용합니다. 명시적 실패·
취소 알림과 브라우저 복귀 URL은 주문이나 자산을 완료하지 않습니다. 과거 inbox에 남은
대기 행은 동일한 알림 재시도에서 불변 사실을 대조한 뒤 같은 원자적 완료 경로로 이어지며,
운영자가 과세 유형을 선택해 수동 완료하는 API와 데이터베이스 함수는 제공하지 않습니다.
