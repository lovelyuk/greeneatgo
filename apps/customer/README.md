...
- Firebase 이메일 가입·인증·로그인 및 비밀번호 재설정
- 회사 초대코드 가입 요청
- 일반 사용자 등록
- 장부 월 이용액·잔여 한도·포인트
- 일반·지원 식권 키움페이 구매
...
Firebase Authentication은 필수입니다. `FIREBASE_ENABLED`는 FCM 알림 사용 여부만
제어하며, `false`여도 Firebase Auth 초기화와 위 Firebase 클라이언트 값은 필요합니다.
Android 표준 빌드 스크립트는 `android/app/google-services.json`에서 공개 클라이언트
값을 읽고, 파일이 없으면 같은 이름의 환경값을 요구합니다.

service role, 키움페이 secret, Firebase Admin 서비스 계정은 앱에 넣지 않습니다.
...
현재 자동 테스트는 기본 환경 안내와 식권 모델 계산 중심입니다. 로그인·QR 카메라·키움페이 WebView·FCM은 실기기 E2E를 병행합니다.
