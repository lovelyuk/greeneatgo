...
const kCocoa = Color(0xFF68813C);
const kOrange = Color(0xFF68813C);
...
class ApiClient {
  ...
  Future<Map<String, dynamic>> createReview(
          int transactionId, int rating, String content, List<String> images) =>
      _request('/reviews', method: 'POST', body: {
...
class DailyMenuClient {
  Future<TodayMenu?> getTodayMenu(String qrToken) async {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      throw Exception(detail?['message'] ?? '오늘 메뉴를 불러오지 못했어요');
    }
    final data = decoded['data'] as Map<String, dynamic>;
    final todayMenuJson = data['today_menu'] as Map<String, dynamic>?;
    return todayMenuJson == null ? null : TodayMenu.fromJson(todayMenuJson);
  }
}
              ...
              onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (_) => CommunityScreen(
                          session: widget.session, initialReviews: false)))),
          ListTile(
              leading: const Icon(Icons.star_outline),
              title: const Text('구매 인증 리뷰'),
              subtitle: const Text('이용 내역에 리뷰를 남겨 보세요'),
              trailing: const Icon(Icons.chevron_right),
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
...
BoxDecoration brandCardDecoration({double radius = 26}) => BoxDecoration(
      ...
      boxShadow: const [
        ...
        BoxShadow(color: Color(0x0D312720), blurRadius: 4, offset: Offset(0, 2))
      ],
    );
