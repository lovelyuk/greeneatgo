...
revoke all on table point_transactions from anon,authenticated,service_role;
grant select,insert on table point_transactions to service_role;
...
-- Handles both point-only (no provider transaction) and verified mixed PG fulfillment atomically/idempotently.
...
language plpgsql security definer set search_path=public as $$
declare o payment_orders%rowtype; v vouchers%rowtype; u app_users%rowtype; duplicate boolean; employee_due bigint;
begin
 select * into o from payment_orders where id=p_order_id for update;
 if not found then raise exception 'ORDER_NOT_FOUND' using errcode='P0001'; end if;
 ...
 values(o.user_id,o.merchant_id,null,o.id,1,employee_due,o.company_id,o.company_subsidy_amount,o.provider_payment_key,coalesce(o.approved_at,now())) on conflict(order_id,issue_index) do nothing;
