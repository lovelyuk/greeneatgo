...
-- payment_orders.provider_payment_key is the authoritative PG transaction key.
...
alter table payment_orders drop constraint if exists payment_orders_refund_snapshot_check;
alter table payment_orders add constraint payment_orders_refund_snapshot_check check (
...
-- The PG cancel is called only after this RPC so two admins cannot issue duplicate cancels.
...
declare o payment_orders%rowtype; r refund_requests%rowtype;
...
begin
  ...
  update payment_orders set refund_account=p_refund_account,updated_at=now() where id=o.id;
  return jsonb_build_object('refund_request_id',r.id,'order_id',o.order_id,
    'provider_payment_key',o.provider_payment_key,'pay_type',o.pay_type,'refund_amount',card_refund,
