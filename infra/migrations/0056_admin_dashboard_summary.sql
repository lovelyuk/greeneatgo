...
drop function if exists public.admin_dashboard_summary(uuid, date, date, uuid, uuid, integer);
...
begin
  ...
  dates as (
    select d::date as civil_date
    from pg_catalog.generate_series(p_period_from, p_period_to, interval '1 day') d
  ),
  daily as (
    select d.civil_date,
      coalesce(sum(f.direction * f.settlement_amount), 0)::bigint as amount,
      coalesce(sum(f.direction), 0)::bigint as count
    from dates d left join facts f
      on f.civil_date = d.civil_date and f.created_at >= v_current_from
    group by d.civil_date
  ),
  series_json as (
    select jsonb_agg(jsonb_build_object('date', civil_date, 'amount', amount, 'count', count)
                     order by civil_date) as value from daily
  )
